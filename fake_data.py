...
def generate_fake_data():
    ...
    password = fake.password(length=random.randint(13, 25), special_chars=True)

    while len(re.findall(r"\d", password)) < 2 or not any(c.isupper() for c in password) or not any(c.islower() for c in password) or not any(c in "!@#$%^&*()_+-=[]{}|;:,.<>?" for c in password):
        password = fake.password(length=random.randint(13, 25), special_chars=True)
